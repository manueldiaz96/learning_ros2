...
DijsktraPlanner::DijsktraPlanner() : Node("dijstra_node")
{
...
void DijsktraPlanner::mapCallback(const nav_msgs::msg::OccupancyGrid::SharedPtr map)
{
    map_ = map;
...
bool DijsktraPlanner::poseOnMap(const GraphNode & node)
{
    return node.x >= 0 && node.x < static_cast<int>(map_->info.width) &&
            node.y >= 0 && node.y < static_cast<int>(map_->info.height);
}

unsigned int DijsktraPlanner::cellToMapIndex(const GraphNode & node){
    return node.x + (node.y * map_->info.width);
}


GraphNode DijsktraPlanner::worldToGrid(const geometry_msgs::msg::Pose & pose)
{
...
geometry_msgs::msg::Pose DijsktraPlanner::gridToWorld(const GraphNode & node){
...
void DijsktraPlanner::goalCallback(const geometry_msgs::msg::PoseStamped::SharedPtr pose)
{
    if(!map_){
...
nav_msgs::msg::Path DijsktraPlanner::plan(const geometry_msgs::msg::Pose & start_pose, const geometry_msgs::msg::Pose & end_pose)
{
    ...
    while(!pending_nodes.empty() && rclcpp::ok())
    {
        ...
        if(worldToGrid(end_pose) == active_node){
            break;
        }
        ...
        visited_map_.data.at(cellToMapIndex(active_node)) = 10;
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<bumperbot_planning::DijsktraPlanner>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}